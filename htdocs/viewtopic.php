...
<div>
	<div class="inbox crumbsplus">
		...
		<div class="nav-wrapper">
			<li><a href="index.php" class="breadcrumb"><?php echo $lang_common['Index'] ?></a></li>
			<li><span>»&#160;</span><a href="viewforum.php?id=<?php echo $cur_topic['forum_id'] ?>" class="breadcrumb"><?php echo pun_htmlspecialchars($cur_topic['forum_name']) ?></a></li>
			<li><span>»&#160;</span><strong><a href="viewtopic.php?id=<?php echo $id ?>" class="breadcrumb"><?php echo pun_htmlspecialchars($cur_topic['subject']) ?></a></strong></li>
		</ul>
		</div>
...
<div id="p<?php echo $cur_post['id'] ?>" class="blockpost<?php echo ($post_count % 2 == 0) ? ' roweven' : ' rowodd' ?><?php if ($cur_post['id'] == $cur_topic['first_post_id']) echo ' firstpost'; ?><?php if ($post_count == 1) echo ' blockpost1'; ?>">
	<div class="mui-panel honbun"><h2><?php if ($user_avatar != '') echo "\t\t\t\t\t\t".'<dd class="postavatar">'.$user_avatar.''."\n"; ?><strong><?php echo $username ?></strong></h2>
	<div class="box">
		<div class="inbox">
			<div class="postbody">
				<div class="postright">
...
<?php if ($signature != '') echo "\t\t\t\t\t".'<div class="postsignature postmsg"><hr />'.$signature.'</div>'."\n"; ?>
		...
		<div class="inbox">
			<div class="postfoot clearb">
				<div class="postfootleft"><span class="conr">#<?php echo ($start_from + $post_count) ?></span> <a href="viewtopic.php?pid=<?php echo $cur_post['id'].'#p'.$cur_post['id'] ?>"><?php echo format_time($cur_post['posted']) ?></a></div>
...
?>
<div id="quickpost" class="blockform mui-panel">
...
?>
<textarea name="req_message" placeholder="本文" rows="7" cols="75" tabindex="<?php echo $cur_index++ ?>"></textarea></label>
						<ul class="bblinks">
...
<?php flux_hook('quickpost_before_submit') ?>
			<p class="buttons"><input type="submit" class="mui-btn mui-btn--raised mui-btn--accent" tabindex="<?php echo $cur_index++ ?>" value="<?php echo $lang_common['Submit'] ?>" accesskey="s" /></p>
		</form>
	</div>
</div>
<?php
